...
files <- dir(path = "/work/APP_NAME/data/",pattern="*\\.rds$")    
...
ui <- fluidPage(
  column(8,offset = 4, titlePanel("RNA-seq data browser")),
  mainPanel(
  h3("GROUP NAME bulk RNA-seq datasets"),
  h6("Made by FT (2022)®")),
...
server <- function(input, output, session) {
  ...
  dataset <- reactive({
    filename <- paste0("/work/APP_NAME/data/",
                       input$Dataset,
                       ".rds")
  ...
  url <- a("GROUP_NAME bulk RNA-seq data", href="INSERT_HYPERLINK_TO_DATABASE_HERE",
           target = "_blank")
  output$tab <- renderUI({
